...
- **macOS**（現ビルドはmacOS向け。AppleScriptは使用しません）
- **Node.js 18以上**
- **Google Chrome** — インストール済み・起動済みで、**ChatGPT Proに既にログインしていること**
- **`playwright-cli` v0.1.8** — グローバルインストール必須。本サーバーが依存している `attach --cdp=chrome` 機能はこのリリース固有で、他バージョンには存在しない可能性があります。
  ```bash
